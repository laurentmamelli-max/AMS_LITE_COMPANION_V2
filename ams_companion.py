...
class StudioBridge(threading.Thread):
    ...
    The archive belongs exclusively to Bambu Studio while it is preparing and
    uploading a job.  Companion is deliberately only an observer: waiting for
    a longer quiet period keeps its parsing and archival copy well outside the
    file-generation window, including on slower Macs and large multi-plate
    projects.
    """

    def __init__(self, app: "Companion", roots: list[Path] | None = None,
                 poll_interval: float = 1.0, stable_seconds: float = 15.0) -> None:
...
class Companion:
    ...
    def detect_vision_object_shapes(self, filename: str) -> dict[str, Any]:
        ...
        This is local visual evidence only.  It has no printer-control path;
        an uncertain result simply produces no overlay in the Vision window.
        """
        runtime = self.state_path.parent / "vision-runtime"
        if runtime.is_dir() and str(runtime) not in sys.path:
            sys.path.insert(0, str(runtime))
        try:
            import vision_linemod
        except ImportError as exc:
            raise RuntimeError("Moteur de formes Vision indisponible") from exc
        if not vision_linemod.available():
            raise RuntimeError("Moteur de formes OpenCV non installé")
        image_path = self._vision_capture_path(filename)
        with self.lock:
            active = self.state.get("active_job")
            recent = self.state.get("bridge", {}).get("recent_import")
        ...
        try:
            layout = vision_linemod.extract_plate_layout(source_path, plate_id, object_map)
            result = vision_linemod.detect_plate_layout(image_path, layout)
        except (OSError, ValueError, RuntimeError) as exc:
            return {"detected": False, "message": str(exc)}
        if not result.get("detected"):
            return {"detected": False, "message": str(result.get("message") or "Aucune forme reconnue")}
        objects = result.get("objects")
        if not isinstance(objects, list):
            return {"detected": False, "message": "Réponse de détection de formes invalide"}
    ...
    def configure(self, data: dict[str, Any]) -> None:
        with self.lock:
            ...
            if camera.get("enabled") and camera.get("certificate_sha256"):
                camera["status"] = (
                    f"Surveillance active — {int(camera.get('capture_views_per_layer', 3) or 3)} vues "
                    f"toutes les {int(camera.get('capture_every_layers', 5) or 5)} couches"
                )
    ...
    def _ensure_camera_print_session_locked(
        ...
        camera.pop("queued_capture_layer", None)
        camera.pop("queued_capture_session_id", None)
        if adopt_ungrouped:
    ...
    def _finalize_camera_print_session_locked(self, result: str, task_id: str, fallback_name: str) -> None:
        ...
        if not session_id or not re.fullmatch(r"print-[a-zA-Z0-9._-]+", folder_name):
            ...
            return
        # A sequence is only useful while the print is moving.  Stop the
        # remaining delayed views at the terminal MQTT event; a frame already
        # in flight may still be stored safely in the completed folder below.
        if str(camera.get("pending_capture_session_id") or "") == session_id:
            camera["pending_capture_layer"] = 0
            camera["pending_capture_session_id"] = ""
            camera.pop("pending_capture_view", None)
        if str(camera.get("queued_capture_session_id") or "") == session_id:
            camera.pop("queued_capture_layer", None)
            camera.pop("queued_capture_session_id", None)
        root = self.state_path.parent / "captures"
    ...
    def _schedule_camera_capture_locked(self, report: dict[str, Any], state: str) -> None:
        ...
        # Do not overwrite a sequence currently reading the camera.  One
        # following layer checkpoint is retained and starts as soon as the
        # current sequence completes, so fast, low layers do not race the
        # daemon or accidentally save an image under the wrong layer number.
    ...
    def _capture_pending_camera(self) -> None:
        """Capture spaced views outside the MQTT lock, without printer control."""
        with self.lock:
            camera = self.state.setdefault("camera", {})
            ...
            while True:
                ...
                try:
                    ...
                    with self.lock:
                        camera = self.state["camera"]
                        camera["pending_capture_view"] = view
                        camera["status"] = f"Vue {view}/{views} impossible : {exc}"
                        self.save()

                with self.lock:
                    camera = self.state["camera"]
                    still_pending = int(camera.get("pending_capture_layer", 0) or 0) == layer
                    more_views = view < views and still_pending
                    if not more_views:
                        camera["pending_capture_layer"] = 0
                        camera["pending_capture_session_id"] = ""
                        camera.pop("pending_capture_view", None)
                ...
                with self.lock:
                    camera = self.state["camera"]
                    queued_layer = int(camera.pop("queued_capture_layer", 0) or 0)
                    queued_session = str(camera.pop("queued_capture_session_id", "") or "")
                    if queued_layer:
                        camera["pending_capture_layer"] = queued_layer
                        camera["pending_capture_session_id"] = queued_session
                        camera["pending_capture_view"] = 0
                        camera["status"] = f"Séquence reprise à la couche {queued_layer}"
                        self.save()
                        continue
                break
        finally:
...
<section><h2>Cartographie G-code (référence manuelle)</h2><p>Les formes du 3MF servent uniquement de référence visuelle. Avec une caméra fixée sur la tête, aucune projection automatique ne peut désigner une pièce de façon fiable.</p><div class="fields"><label>Largeur G-code du plateau (mm)<input id="bedX" type="number" min="1" max="500" step="1" value="180"></label><label>Profondeur G-code du plateau (mm)<input id="bedY" type="number" min="1" max="500" step="1" value="180"></label></div><button class="secondary" onclick="startCalibration()">Réglage manuel de secours…</button><p id="calibrationStatus" class="muted">La cartographie reste un outil de contrôle humain, jamais une détection automatique.</p></section>
...
function installVisionEngine(){if(window.webkit?.messageHandlers?.companion){window.webkit.messageHandlers.companion.postMessage('installVisionEngine');setCalibrationStatus('Installation locale du moteur de formes lancée. Attends sa fin, puis relance la recherche.',false);return}setCalibrationStatus('Dans la version locale, double-clique Installer_Moteur_Vision.command puis relance la recherche.',false)}
...
async function startAutoCalibration(){if(!currentCapture||modalEl.hidden){setCalibrationStatus('Ouvre une capture, puis lance la reconnaissance de formes.',true);return}await detectObjectShapes();setCalibrationStatus(currentShapeObjects.length?'Formes 3MF reconnues pour cette capture. Vérifie les contours avant toute décision.':'Aucune forme assez sûre : aucun contour n’est affiché.',!currentShapeObjects.length)}
