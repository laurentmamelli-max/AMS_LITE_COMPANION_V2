...
class Companion:
    ...
    def register_vision_capture(self, reference_filename: str, current_filename: str) -> dict[str, Any]:
        """Register a manually calibrated reference capture onto a new frame.

        This only derives an image transform.  It never controls the printer,
        and rejects malformed transforms so the UI can keep overlays hidden.
        """
        reference_path = self._vision_capture_path(reference_filename)
        current_path = self._vision_capture_path(current_filename)
        try:
            result = subprocess.run(
                [str(self._vision_calibration_helper()), "--register", str(reference_path), str(current_path)],
                check=True, capture_output=True, timeout=15, text=True,
            )
            payload = json.loads(result.stdout)
        except subprocess.TimeoutExpired as exc:
            raise RuntimeError("Recalage dynamique expiré") from exc
        except (subprocess.CalledProcessError, json.JSONDecodeError) as exc:
            raise RuntimeError("Recalage dynamique impossible sur cette capture") from exc
        matrix = payload.get("matrix") if isinstance(payload, dict) else None
        if not payload.get("registered") or not isinstance(matrix, list) or len(matrix) != 3:
            return {"registered": False, "message": str(payload.get("message") or "Recalage non trouvé")}
        clean: list[list[float]] = []
        for row in matrix:
            if not isinstance(row, list) or len(row) != 3:
                return {"registered": False, "message": "Matrice de recalage invalide"}
            values: list[float] = []
            for value in row:
            ...
            clean.append(values)
        if abs(clean[2][2]) < 1e-8:
            return {"registered": False, "message": "Matrice de recalage dégénérée"}
        return {"registered": True, "matrix": clean}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        try:
            ...
            elif path == "/api/vision/captures/register":
                body = self.json_body()
                self.send_json(self.app.register_vision_capture(
                    str(body.get("reference_file") or ""), str(body.get("current_file") or ""),
                ))
...
<section><h2>Projection dynamique des objets</h2><p>Les contours rouges utilisent les positions vérifiées dans le G-code. Régle une capture où le plateau est lisible : le Centre Vision recale ensuite cette perspective sur chaque nouvelle image, sans commande envoyée à l’imprimante.</p><div class="fields"><label>Largeur G-code du plateau (mm)<input id="bedX" type="number" min="1" max="500" step="1" value="180"></label><label>Profondeur G-code du plateau (mm)<input id="bedY" type="number" min="1" max="500" step="1" value="180"></label></div><button class="secondary" onclick="startAutoCalibration()">Reprendre le suivi sur cette capture</button><button class="secondary" onclick="startCalibration()">Régler une capture de référence…</button><p id="calibrationStatus" class="muted">Les images trop occultées restent volontairement sans contour : aucune projection n’est inventée.</p></section>
<section><h2>Captures par impression</h2><div id="gallery"></div></section></main>
<div id="captureModal" class="capture-modal" hidden onclick="if(event.target===this)closeCapture()"><section class="capture-dialog"><button class="secondary capture-close" onclick="closeCapture()">Fermer</button><h2 id="captureTitle">Capture</h2><button id="autoCalibrateCaptureButton" onclick="startAutoCalibration()">Reprendre le suivi ici</button><button id="calibrateCaptureButton" onclick="startCalibration()">Régler cette capture comme référence</button><div id="calibrationActions" class="calibration-actions" hidden><button class="secondary" onclick="undoCalibrationPoint()">Annuler le dernier coin</button><button class="secondary" onclick="skipCalibrationCorner()">Ce coin est hors champ</button><button class="danger" onclick="startCalibration()">Recommencer</button></div><p id="overlayNotice" class="muted"></p><div id="captureStage" class="capture-stage"><img id="captureLarge" alt="Capture agrandie"><svg id="captureOverlay" viewBox="0 0 1 1" preserveAspectRatio="none"></svg></div><div id="objectLegend" class="object-legend" aria-label="Légende des objets cartographiés"></div><p id="calibrationPoints" class="calibration-points"></p><div id="captureInfo" class="capture-info"></div></section></div>
<script>
...
let visionState=null,currentCapture=null,currentPlatePoints=null,currentPlateMessage='',calibrationMode=false,calibrationPoints=[],calibrationEdgePoints=[],hiddenCornerIndex=-1,selectedObjectIndex=-1;const calibrationKey='ams-lite-vision-plate-calibration-v1';
...
function openCapture(index){currentCapture=(window.visionCaptures||[])[index];if(!currentCapture)return;currentPlatePoints=null;currentPlateMessage='Recalage dynamique de la perspective…';calibrationMode=false;calibrationPoints=[];calibrationEdgePoints=[];hiddenCornerIndex=-1;selectedObjectIndex=-1;titleEl.textContent=`Capture · couche ${currentCapture.layer}`;largeEl.src=captureURL(currentCapture.file);largeEl.alt=`Capture de la couche ${currentCapture.layer}`;infoEl.innerHTML=`<div><b>Impression</b>${esc(currentCapture.print_name||'Impression en cours')}</div><div><b>Couche</b>${currentCapture.layer}</div><div><b>Date</b>${esc(currentCapture.captured_at)}</div><div><b>Fichier</b>${esc(currentCapture.file)}</div>`;modalEl.hidden=false;renderCalibrationProgress();largeEl.onload=()=>{drawOverlay();trackPerspective()};drawOverlay()}
function transformPoint(matrix,point){if(!Array.isArray(matrix)||matrix.length!==3||!Array.isArray(point)||point.length!==2)return null;const x=Number(point[0]),y=Number(point[1]),d=Number(matrix[2]?.[0])*x+Number(matrix[2]?.[1])*y+Number(matrix[2]?.[2]);if(!Number.isFinite(d)||Math.abs(d)<1e-8)return null;const u=(Number(matrix[0]?.[0])*x+Number(matrix[0]?.[1])*y+Number(matrix[0]?.[2]))/d,v=(Number(matrix[1]?.[0])*x+Number(matrix[1]?.[1])*y+Number(matrix[1]?.[2]))/d;return Number.isFinite(u)&&Number.isFinite(v)?[u,v]:null}
function usableTrackedPlate(points){if(!Array.isArray(points)||points.length!==4||points.some(point=>!Array.isArray(point)||point.length!==2||point.some(value=>!Number.isFinite(value)||value<-1||value>2)))return false;const twice=points.reduce((sum,point,index)=>{const next=points[(index+1)%points.length];return sum+point[0]*next[1]-next[0]*point[1]},0);return Math.abs(twice)>=0.004}
async function trackPerspective(){if(!currentCapture||modalEl.hidden)return;const file=currentCapture.file,saved=loadCalibration();if(!saved?.reference_file){currentPlatePoints=null;currentPlateMessage='Choisis une capture où le plateau est visible puis règle-la comme référence. Les anciens réglages sans image source ne sont pas réutilisés.';drawOverlay();return}if(saved.reference_file===file){currentPlatePoints=saved.points;currentPlateMessage='Capture de référence : projection issue du réglage manuel.';drawOverlay();return}try{const result=await api('/api/vision/captures/register',{method:'POST',body:JSON.stringify({reference_file:saved.reference_file,current_file:file})});if(!currentCapture||currentCapture.file!==file)return;const points=Array.isArray(result.matrix)?saved.points.map(point=>transformPoint(result.matrix,point)):[];if(!result.registered||!usableTrackedPlate(points)){currentPlatePoints=null;currentPlateMessage=result.message||'Perspective non exploitable sur cette image : aucun contour affiché.'}else{currentPlatePoints=points;currentPlateMessage='Perspective recalée depuis la capture de référence. Vérifie les contours avant toute décision.'}drawOverlay()}catch(error){currentPlatePoints=null;currentPlateMessage=error.message||'Recalage dynamique impossible.';drawOverlay()}}
...
function saveCalibration(points,extendedCorner=null,method='manual'){const width=Math.max(1,Number(bedXEl.value)||180),height=Math.max(1,Number(bedYEl.value)||180),referenceFile=currentCapture?.file||null;localStorage.setItem(calibrationKey,JSON.stringify({points,width,height,reference_file:referenceFile,extended_corner:extendedCorner,method}));currentPlatePoints=points;currentPlateMessage='Capture de référence enregistrée localement.';calibrationMode=false;const message=extendedCorner===null?'Capture de référence enregistrée. Les prochaines images seront recalées à partir de celle-ci.':`Capture de référence enregistrée : le coin ${calibrationLabels[extendedCorner]} a été reconstruit par l’intersection des deux bords. Vérifie que les contours suivent les pièces.`;setCalibrationStatus(message);renderCalibrationProgress();drawOverlay()}
function downloadCalibrationSheet(){if(window.webkit?.messageHandlers?.companion){window.webkit.messageHandlers.companion.postMessage('downloadCalibrationSheet');setCalibrationStatus('Génération de la planche dans Téléchargements…');return}const link=document.createElement('a');link.href=`/api/vision/calibration-sheet.pdf?token=${encodeURIComponent(token)}`;link.download='ams-companion-calibration-180mm.pdf';document.body.append(link);link.click();link.remove();setCalibrationStatus('Planche téléchargée. Imprime-la à 100 % sans ajustement, puis pose-la à plat sur le plateau vide.')}
...
async function startAutoCalibration(){if(!currentCapture||modalEl.hidden){setCalibrationStatus('Ouvre une capture, puis relance le suivi dynamique.',true);return}await trackPerspective();setCalibrationStatus(currentPlatePoints?'Perspective recalée pour cette capture. Vérifie les contours avant toute décision.':'Perspective non exploitable : aucun contour n’est affiché.',!currentPlatePoints)}
