...
<h1>AMS Lite Companion V2</h1><div class="sub">Compteur local v3.0.2 — alertes locales, exclusion uniquement manuelle et confirmée.</div><div id="msg"></div>
...
<script>
const reportsCard=document.querySelector('.reports-card'),shutdownCard=document.querySelector('.shutdown-card');if(reportsCard&&shutdownCard){shutdownCard.after(reportsCard);reportsCard.style.order='99'}
...
<section><h2>Projection des objets cartographiés</h2><p>Les contours rouges utilisent les positions vérifiées dans le G-code. Ce n’est pas une reconnaissance visuelle automatique : la projection devient précise après une calibration unique du plateau.</p><div class="fields"><label>Largeur G-code du plateau (mm)<input id="bedX" type="number" min="1" max="500" step="1" value="180"></label><label>Profondeur G-code du plateau (mm)<input id="bedY" type="number" min="1" max="500" step="1" value="180"></label></div><button class="secondary" onclick="startCalibration()">Calibrer le plateau sur une capture…</button><button class="danger" onclick="clearCalibration()">Effacer la calibration</button><p id="calibrationStatus" class="muted">Ouvre d’abord une capture, puis clique les quatre coins du plateau.</p></section>
<section><h2>Captures par impression</h2><div id="gallery"></div></section></main>
<div id="captureModal" class="capture-modal" hidden onclick="if(event.target===this)closeCapture()"><section class="capture-dialog"><button class="secondary capture-close" onclick="closeCapture()">Fermer</button><h2 id="captureTitle">Capture</h2><button id="calibrateCaptureButton" onclick="startCalibration()">Calibrer ce plateau</button><p id="overlayNotice" class="muted"></p><div id="captureStage" class="capture-stage"><img id="captureLarge" alt="Capture agrandie"><svg id="captureOverlay" viewBox="0 0 1 1" preserveAspectRatio="none"></svg></div><p id="calibrationPoints" class="calibration-points"></p><div id="captureInfo" class="capture-info"></div></section></div>
<script>
const token=__API_TOKEN__,statusEl=document.getElementById('status'),enabledEl=document.getElementById('enabled'),fingerprintEl=document.getElementById('fingerprint'),metaEl=document.getElementById('meta'),galleryEl=document.getElementById('gallery'),modalEl=document.getElementById('captureModal'),titleEl=document.getElementById('captureTitle'),largeEl=document.getElementById('captureLarge'),overlayEl=document.getElementById('captureOverlay'),overlayNoticeEl=document.getElementById('overlayNotice'),infoEl=document.getElementById('captureInfo'),calibrationStatusEl=document.getElementById('calibrationStatus'),calibrationPointsEl=document.getElementById('calibrationPoints'),bedXEl=document.getElementById('bedX'),bedYEl=document.getElementById('bedY'),layerCounterEl=document.getElementById('layerCounter');
let visionState=null,currentCapture=null,calibrationMode=false,calibrationPoints=[];const calibrationKey='ams-lite-vision-plate-calibration-v1';
...
function drawOverlay(){overlayEl.innerHTML='';if(!currentCapture)return;const objects=objectsFor(currentCapture),saved=calibration();if(!objects.length){overlayNoticeEl.textContent='Cette capture ancienne ne contient pas sa cartographie G-code. Ouvre une capture de l’impression en cours, identifiée « objet(s) cartographié(s) » dans la galerie.';return}if(!saved){overlayNoticeEl.textContent=`${objects.length} objet(s) cartographié(s). Clique « Calibrer ce plateau » pour afficher leurs contours rouges.`;return}const h=homography(saved.points,Number(saved.width)||180,Number(saved.height)||180);if(!h){overlayNoticeEl.textContent='Calibration invalide : recommence la sélection des quatre coins.';return}let count=0,svg='';for(const object of objects){const b=object?.bounds_xy;if(!b)continue;const corners=[[b.min_x,b.min_y],[b.max_x,b.min_y],[b.max_x,b.max_y],[b.min_x,b.max_y]].map(([x,y])=>project(h,Number(x),Number(y)));if(corners.some(point=>!point||!Number.isFinite(point[0])||!Number.isFinite(point[1])))continue;const points=corners.map(point=>`${point[0]},${point[1]}`).join(' '),label=corners[0];svg+=`<polygon points="${points}" fill="#ff2b2240" stroke="#ef1f18" stroke-width="0.008" vector-effect="non-scaling-stroke"/><text x="${label[0]}" y="${label[1]}" fill="#ef1f18" font-size="0.035" font-weight="700" stroke="white" stroke-width="0.008" paint-order="stroke">${esc(object.label||object.id)}</text>`;count++}overlayEl.innerHTML=svg;overlayNoticeEl.textContent=count?`${count} contour(s) rouges projetés depuis le G-code — pas une détection visuelle automatique.`:'Les objets cartographiés n’ont pas de zone XY exploitable.'}
function renderCalibrationProgress(){if(!calibrationMode){calibrationPointsEl.textContent='';return}const labels=['X0 / Y0','Xmax / Y0','Xmax / Ymax','X0 / Ymax'];calibrationPointsEl.innerHTML=`<b>Calibration en cours :</b> ${calibrationPoints.length}/4 coin(s) sélectionné(s). Prochain clic : ${labels[calibrationPoints.length]||'enregistrement…'}.`}
function openCapture(index){currentCapture=(window.visionCaptures||[])[index];if(!currentCapture)return;calibrationMode=false;calibrationPoints=[];titleEl.textContent=`Capture · couche ${currentCapture.layer}`;largeEl.src=captureURL(currentCapture.file);largeEl.alt=`Capture de la couche ${currentCapture.layer}`;infoEl.innerHTML=`<div><b>Impression</b>${esc(currentCapture.print_name||'Impression en cours')}</div><div><b>Couche</b>${currentCapture.layer}</div><div><b>Date</b>${esc(currentCapture.captured_at)}</div><div><b>Fichier</b>${esc(currentCapture.file)}</div>`;modalEl.hidden=false;renderCalibrationProgress();largeEl.onload=drawOverlay;drawOverlay()}
function closeCapture(){modalEl.hidden=true;calibrationMode=false;calibrationPoints=[];largeEl.removeAttribute('src');overlayEl.innerHTML=''}
function startCalibration(){if(!currentCapture||modalEl.hidden){setCalibrationStatus('Ouvre une capture dans la galerie, puis lance la calibration.',true);return}calibrationMode=true;calibrationPoints=[];setCalibrationStatus('Clique les coins dans l’ordre indiqué sous l’image.');renderCalibrationProgress();overlayNoticeEl.textContent='Calibration : les clics servent uniquement à projeter le G-code sur cette caméra.'}
largeEl.addEventListener('click',event=>{if(!calibrationMode)return;const box=largeEl.getBoundingClientRect(),x=(event.clientX-box.left)/box.width,y=(event.clientY-box.top)/box.height;if(x<0||x>1||y<0||y>1)return;calibrationPoints.push([x,y]);renderCalibrationProgress();if(calibrationPoints.length!==4)return;const width=Math.max(1,Number(bedXEl.value)||180),height=Math.max(1,Number(bedYEl.value)||180);localStorage.setItem(calibrationKey,JSON.stringify({points:calibrationPoints,width,height}));calibrationMode=false;setCalibrationStatus('Calibration enregistrée localement. Les contours rouges apparaissent maintenant sur les captures cartographiées.');renderCalibrationProgress();drawOverlay()});
